...
export default function App() {
  ...
  const handleDownloadPDF = (practiceSet: PracticeSet) => {
    ...
    const addFooter = () => {
      ...
      doc.text(
        `Generated by Elimu AI  •  elimuaiai.co.ke  •  Page ${pageCount}`,
        pageWidth / 2,
        pageHeight - 10,
        { align: 'center' }
      );
    ...
    doc.text("Elimu AI", margin, 15);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100, 100, 100);
    doc.text("elimuaiai.co.ke", margin + 15, 15);
    ...
    doc.setFontSize(22);
    doc.setFont("helvetica", "bold");
    doc.text(`${practiceSet.subject}: ${practiceSet.topic}`, pageWidth / 2, 35, { align: 'center' });
    
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text(`${practiceSet.level} - ${practiceSet.difficulty} Difficulty`, pageWidth / 2, 45, { align: 'center' });

    // Student Details
    doc.setFontSize(11);
    doc.text("Name: ________________________________________________________________________", margin, 60);
    doc.text("Class: _________________________________________________________________________", margin, 70);
    ...
    // Instructions
    doc.setFont("helvetica", "bold");
    doc.text("Instructions: Answer all questions. Show your working where applicable.", pageWidth / 2, 85, { align: 'center' });
    ...
    const parts = practiceSet.content.split(/---|\n#{1,3}\s*Answers|Majibu|Answers & Marking Scheme/i);
    const questionsContent = parts[0];
    const answersContent = parts.slice(1).join('\n---\n');

    const processText = (text: string, currentY: number) => {
      ...
      lines.forEach(line => {
        ...
        if (line.startsWith('###') || line.startsWith('##') || line.startsWith('#')) {
          ...
          if (localY + (wrappedHeading.length * 7) > pageHeight - 30) {
            addFooter();
            doc.addPage();
            localY = 20;
          }

          doc.text(wrappedHeading, pageWidth / 2, localY, { align: 'center' });
          localY += (wrappedHeading.length * 7) + 5;
          return;
        }

        // Handle MCQ options (indented)
        let xPos = margin;
        if (line.trim().match(/^[A-D][\.\)]/)) {
          xPos += 10;
        }

        const wrappedLines = doc.splitTextToSize(line, contentWidth - (xPos - margin));
        
        if (localY + (wrappedLines.length * 7) > pageHeight - 30) {
          addFooter();
          doc.addPage();
          localY = 20;
        }

        doc.text(wrappedLines, xPos, localY);
        localY += (wrappedLines.length * 7) + 3; // Extra space between items
      });
      return localY;
    };

    y = processText(questionsContent, y);

    // Answers Section on New Page
    if (practiceSet.includeAnswers && answersContent.trim()) {
      addFooter();
      doc.addPage();
      y = 25;
      doc.setFont("helvetica", "bold");
      doc.setFontSize(16);
      doc.text("ANSWERS & MARKING SCHEME", pageWidth / 2, y, { align: 'center' });
      y += 15;
      processText(answersContent, y);
    }

    addFooter();
    
    const cleanTopic = practiceSet.topic.replace(/[^a-zA-Z0-9]/g, '');
    const filename = `${practiceSet.subject}-${cleanTopic}-${practiceSet.level.replace(/\s+/g, '')}-${practiceSet.difficulty}-${new Date().toLocaleDateString('en-GB').replace(/\//g, '')}.pdf`;
    doc.save(filename);
